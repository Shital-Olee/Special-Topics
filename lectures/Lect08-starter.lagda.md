...
```
  ...
  EqualsList (x :: l1) (x₁ :: l2) = EqualsList l1 l2
...
addx : (l : List Char) (x : Char) 
    (sp : Splitting l)
    → Splitting (x :: l)
addx l x (f , b , appenfb) = x :: f , b , refl-char x, appendfb
...
addx-all l x (sp :: splits) = addx l x sp :: addx-all l x splits
  ...
  split [] = ([], ([] , <>)) :: []
  split (x :: xs) = ([] , x :: xs , refl-list (x :: xs)) :: addx-all xs x (split xs)
...
```
  ...
  match (Lit x) s = case (decide-list-equality1 s x) where
    ...
    case None = None
  match Wild s = Some <> 
  match (r · r₁) s = {!loop oover split s, checking them one by one!}
  match (r1 ∨ r2) s = case (match r1 s) where
    case : Maybe (s ∈L r1) → Maybe (Either (s ∈L r1) (s ∈L r2))
    case (Some sinr1) = Some (Inl sinr1)
    case None = case2 (match r2 s) where
      case2 : Maybe (s ∈L r2) → Maybe (Either (s ∈L r1) (s ∈L r2))
      case2 (Some sinr2) = Some (Inr sinr2)
      case2 None = None 
