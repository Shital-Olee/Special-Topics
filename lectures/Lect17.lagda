...
```
  ...
  mutual
    ...
    data _∈L_ : List Char → RegExp → Set where
      ...
      In+1 : (s : List Char) (r : RegExp) → s ∈L r → s ∈L (r +)
...
```
  ...
  -- splits into s1 s2 ... sn where s1 ∈L r1 and s2 .. sn ∈L k=r2...rn  
  ...
  mutual
    ...
    ...                    | Some ( (f1 , b' , Refl _) , f1inr1 , In:: _ (f2 , b , Refl _) _ _ (f2inr2 , bink)) =
                                    Some ((append f1 f2 , b , append-assoc f1 f2 b) ,
                                          ...
                                          bink)
    match (Lit c) k [] = None
    match (Lit c) k (x :: s) with equalChar x c
    ...                         | Inr _ = None
    match (Lit c) k (c :: s)    | Inl (Refl _) with matchstack k s
    ...                                           | Some ink = Some ((c :: [] , s , Refl _) , InLit _ , ink)
    ...                                           | None = None
    match (r1 ∨ r1') k s with match r1 k s 
    ...                    | Some (sp , finr1 , bink) = Some (sp , In∨Left _ _ _ finr1 , bink)
    ...                    | None with match r1' k s 
    ...                              | Some (sp , finr1' , bink) = Some (sp , In∨Right _ _ _ finr1' , bink)
    ...                              | None   = None
    match (r +) k s with match r k s
    ...                           | Some (sp , finr , bink) = Some (sp , In+1 _ _ finr , bink)
    ...                           | None with match r ((r +) :: k) s
    ...                                     | Some ((f1 , _ , Refl _) , (f1inr , In:: _ (f2 , b , Refl _) _ _ (f2inr+ , bink))) =
                                                        Some ( (append f1 f2 , b , append-assoc f1 f2 b) , In+Split _ _ (f1 , f2 , Refl _) (f1inr , f2inr+) , bink)
    ...                                     | None = None


    matchstack : (k : Stack) (s : List Char) → Maybe (s ∈S k)
    matchstack [] [] = Some In[]
    matchstack [] (x :: s) = None
    matchstack (r :: k) s with match r k s
    ...                      | Some (sp , finr , bink) = Some (In:: _ sp _ _ (finr , bink)) 
    ...                      | None = None

  change-string : (r : RegExp) (s1 s2 : List Char) → Equals _ s1 s2 → s1 ∈L r → s2 ∈L r
  change-string r s1 s2 (Refl _) p = p

  real-match : (r : RegExp) (s : List Char) → Maybe (s ∈L r)
  real-match r s with match r [] s
  real-match r _ | Some ((s , [] , Refl _) , finR , In[]) =
             Some (change-string _ _ _ (append-rh-[] s) finR)
  real-match r _ | None = None
```
